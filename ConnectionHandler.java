...
public class ConnectionHandler {
				...
				if (key.isAcceptable()) {
					...
					try {
						...
						if (key.isReadable()) {
							...
							// Next, pass the buffer off to handleMessage which will, y'know, handle the
							// message
	...
	// Returns a ByteBuffer with the contents from cchannel's socket. ByteBuffer is
	// already flipped
	 ...
	 * 
	 * The first short of each initial message will tell handleMessage what type of
	 * message it is. For example, if type = 1 means its a message to a chat room,
	 * then the logic for handling messages to chat room should be contained in case
	 * 1
	 * 
	 * HOW TO DO: 1) Find a free message type (i.e. if type = 37 hasn't been used,
	 * you can use that type) 2) Create a function that handles the logic for that
	 * specific message type 3) Call that function from the case for your chosen
	 * type
	 * 
	 * NOTE: Make sure each case you write is followed by a break statement. If you
	 * need to terminate the connection with the user that sent the message, put
	 * "return false;" at the end of your case
	 */
	...
	private boolean handleMessage(ByteBuffer message, SocketChannel sock) throws IOException {
		...
		case 666:
			...
			break;
		case 669:
			Server.handleViewPublicChats(message, sock);
			break;
		case 670:
			Server.handleAddPublicChatUser(message, sock);
			break;
		case 671:
			Server.handleGetUID(message, sock);
			break;
		case 672:
			Server.handleCreatePublicChat(message, sock);
			break;
